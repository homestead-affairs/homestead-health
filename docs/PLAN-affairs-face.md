...
Struck lines carry `(#PR, released X.Y.Z)`; PR numbers and release versions
are read from this repo's own `git log`/`CHANGELOG.md`, not asserted from
memory. A bite named in the plan but not yet landed keeps its plain text —
this file does not get ahead of what merged.
