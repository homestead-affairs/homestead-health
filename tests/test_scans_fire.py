...
is shaped like a violation-scanner and asserts that some planted-violation
test in the *same file* runs that helper — directly, or through another
helper it calls.

**Discovery is structural, not a naming convention.** The first cut of this
file looked for helpers whose *name* carried one of the repo's stems
(`_record_synced_offenders`, `_reads_a_jsonl_path_with_bare_json_loads`) or
whose body called `ast.parse`/`ast.walk`, and it required a leading
underscore. The X7 audit planted two scans it could not see and both walked
through: a public `check_payload_reach(...)` that walks `ast` (missed on the
underscore rule alone — its name does carry `check`, which the stem list did
not hold either), and a grep-shaped `forbidden_word_hits(path)` that reads a
file and asks `"payload" in text` with no `ast` and no matching stem at all.
So the rule below is what a scan *does*:
...
it: they still catch `_toplevel_imports`-adjacent helpers that hand their
work to a caller. Discovery is a union, so it is strictly wider than the
convention it replaced, never narrower.

**Having a plant means a plant test calls the scan.** The first cut read
names and docstrings only: a test called `test_the_guard_fires` that never
touched the guard cleared the whole file. Now a helper counts as planted
only when a `test_*` function whose name or docstring carries `plant`,
`fires` or `catches` reaches it — through the module's own helpers as well as
directly, so a plant that calls `_modules_missing_from_status_table()` also
clears the `_module_status_section()` it is built on. Both halves are planted
...
inline in a test body rather than as a helper are invisible to it too — the
X7 audit's answer to that was to lift three of them (`_listen_calls`,
`_expanduser_calls`, `_undeclared_imports` in `tests/test_invariants_seat.py`)
into helpers so they fall under this file at all.
"""
...
def _is_scan_helper(node: ast.FunctionDef) -> bool:
    ...
    `check_payload_reach` is the same scan with a different name, and the
    audit planted exactly that.
    """
    name = node.name
...
def _helpers_the_plants_exercise(source: str) -> set[str]:
    ...
    own helpers, because a plant that calls `_modules_missing_from_status_
    table()` has exercised the `_module_status_section()` underneath it just
    as surely as if it had called it by hand.
    """
...
def test_the_meta_scan_finds_an_ast_shaped_scan_whatever_it_is_named(tmp_path):
    """Planted: a scan that walks `ast` under a **public** name. The first cut
    of this file required a leading underscore, so `check_payload_reach` —
    the same scan, spelled the way a helper meant to be imported is spelled —
    was invisible to it and its missing plant went unreported."""
    source = _write(
        tmp_path,
...
def test_the_meta_scan_finds_a_grep_shaped_scan(tmp_path):
    ...
    `in`. The repo has one of these already (`test_docs_drift.py::_contains`),
    and a discovery rule that only knew about `ast` would clear a file whose
    only guard is this shape."""
    source = _write(
        tmp_path,
...
def test_the_plant_check_requires_the_plant_test_to_call_the_scan(tmp_path):
    ...
    *named* `test_the_scan_fires` and whose body never touches the scan must
    still be reported — the exact hole in the first cut of this file, which
    read names and docstrings and nothing else."""
    source = _write(
        tmp_path,
...
def test_a_plant_that_calls_the_scan_through_a_helper_clears_it(tmp_path):
    """And not over-strict: the repo's real plants call a wrapper
    (`_modules_missing_from_status_table`) rather than the parser underneath
    it (`_module_status_section`). Reaching a helper through another helper
    is having exercised it, so neither may be reported."""
    source = _write(
        tmp_path,
...
def test_the_plant_check_does_not_fire_on_a_real_guarded_file():
    ...
    `tests/test_ledger_seam.py` defines two scans and plants both, so the
    meta-scan must clear it — or it would be crying wolf on the very files it
    exists to clear."""
