...
``<household_root>/nestor-health.db``.  It is the host's adapter — distinct
...
household root, never a global or a temp: Nestor's entity and decision memory
for a matter is a household artefact, not a session artefact.
...
def store_path(household_root: Path | None = None) -> Path:
    """The path where Nestor's health store lives — inside the household root."""
    root = Path(household_root) if household_root is not None else paths.home()
    return root / "nestor-health.db"


def get_store(household_root: Path | None = None) -> Any:
    """A SqliteStore at ``<household_root>/nestor-health.db``, cached per process.
